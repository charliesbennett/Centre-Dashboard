...
export default function ProgrammesTab({ groups, progStart, progEnd, centre, excDays, setExcDays, excursions = [], progGrid, setProgGrid, settings, saveSetting, readOnly = false, isHeadOffice = false, centres = [] }) {
  ...
  const classify = (text, ds) => {
    ...
    if (t.includes("arrival")) return PILL.arrival;
    if (t.includes("depart")) return PILL.depart;
    if (t.includes("lesson") || t.includes("english test")) return PILL.lesson;
    if (t.includes("exc")) return PILL.exc;
    if (t.includes("evening") || t === "ee" || t.includes(" ee") || t.includes("disco") || t.includes("quiz") || t.includes("movie") || t.includes("trashion") || t.includes("speed dating") || t.includes("paparazzi")) return PILL.evening;
    if (t.includes("activity") || t.includes("multi-activity") || t.includes("sports") || t.includes("games") || t.includes("orientation") || t.includes("welcome") || t.includes("chosen programme")) return PILL.activity;
    if (t.includes("free time") || t.includes("optional")) return PILL.freetime;
    if (ds && (excDays[ds] || isWeekend(new Date(ds + "T12:00:00")))) return PILL.exc;
  ...
  return (<div>
    ...
    {viewMode==="all" && <div style={{margin:"0 4px 16px",overflow:"auto",maxHeight:"calc(100vh - 260px)",background:B.card,borderRadius:12,border:"1px solid "+B.border,boxShadow:"0 2px 12px rgba(28,48,72,0.07)"}}>
      <table style={{minWidth:1200,width:"100%",borderCollapse:"collapse",fontSize:10}}>
        <thead style={{position:"sticky",top:0,zIndex:2}}>
          <tr>
            ...
            <th style={{...thStyle,width:90,maxWidth:90,overflow:"hidden",whiteSpace:"nowrap",position:"sticky",left:100,zIndex:1,background:B.bg,color:B.textMuted,backgroundImage:"none"}}>Group</th>
          ...
          <tr>
            <th style={{...thStyle,position:"sticky",left:0,zIndex:1,background:B.bg,backgroundImage:"none"}}></th>
            <th style={{...thStyle,position:"sticky",left:100,zIndex:1,background:B.bg,backgroundImage:"none"}}></th>
            <th style={thStyle}></th>
            <th style={thStyle}></th>
        ...
        <tbody>
          ...
          groups.map(g=><tr key={g.id} style={{borderBottom:"1px solid "+B.borderLight}}>
            ...
            {dates.map(d=>slots.map(sl=>{const s=dayKey(d),key=g.id+"-"+s+"-"+sl,val=grid[key],on=inRange(s,g.arr,g.dep),cls=classify(val,s),isEd=editingCell===key,isQP=quickPickCell===key;
              return<td key={key} onClick={(e)=>handleCellClick(key,sl,on,e)} onDoubleClick={()=>!readOnly&&on&&startEdit(key,val)} style={{padding:"3px",border:"none",verticalAlign:"top",minWidth:sl==="Eve"?86:110,background:"#1c3048",cursor:on?"pointer":"default",position:"relative",height:96}}>
                {isEd?<input autoFocus value={editValue} onChange={e=>setEditValue(e.target.value)} onBlur={commitEdit} onKeyDown={e=>e.key==="Enter"&&commitEdit()} style={{width:"100%",height:"100%",fontSize:12,padding:"6px 8px",border:"none",borderRadius:4,fontFamily:"inherit",background:"#fff",color:"#1c3048",boxSizing:"border-box"}}/>:
                (val&&on)?<div style={{background:cls.bg,borderRadius:4,height:"100%",overflow:"hidden",boxSizing:"border-box"}} title={val}><div style={{color:cls.color,fontSize:12,fontWeight:700,padding:"6px 8px",lineHeight:1.4,wordBreak:"break-word"}}>{val}</div></div>:
      ...
      <div style={{padding:"8px 12px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
        ...
        <span style={{fontSize:9,color:B.textMuted,marginLeft:"auto"}}>{isMinistay ? "Click cell for quick-pick \u00b7 Double-click for custom text \u00b7 Click date headers for exc days" : "Double-click to edit \u00b7 Click date headers for exc days \u00b7 Lessons follow Wk1 slot, flip each week"}</span>
      </div>
    </div>}
      ...
      {selGroup && <div style={{padding:"0 8px 16px",overflowX:"auto"}}>
        ...
        <TableWrap><table style={{minWidth:1200,width:"100%",borderCollapse:"collapse",fontSize:10}}>
          <thead><tr><th style={{...thStyle,width:30}}></th>
            {dates.filter(d=>inRange(dayKey(d),selGroup.arr,selGroup.dep)).map(d=>{const s=dayKey(d),we=isWeekend(d),exc=excDays[s];return<th key={s} style={{...thStyle,textAlign:"center",minWidth:100,background:exc?"#fef3c7":we?"#fee2e2":B.ice}}>
              <div style={{fontSize:7,color:B.textMuted}}>{fmtDate(d)}</div>
              <div style={{fontWeight:800,fontSize:9,color:we?"#b91c1c":B.text}}>{dayName(d)}</div>
              {exc&&<div style={{fontSize:7,color:"#b45309",fontWeight:800}}>{exc==="Full"?"FULL EXC":"HALF EXC"}</div>}
            </th>;})}
          </tr></thead>
          <tbody>{slots.map(sl=><tr key={sl} style={{borderBottom:"1px solid "+B.borderLight}}>
            ...
            {dates.filter(d=>inRange(dayKey(d),selGroup.arr,selGroup.dep)).map(d=>{const s=dayKey(d),key=selGroup.id+"-"+s+"-"+sl,val=grid[key],cls=classify(val,s),isEd=editingCell===key,isQP=quickPickCell===key;
              return<td key={key} onClick={(e)=>handleCellClick(key,sl,true,e)} onDoubleClick={()=>!readOnly&&startEdit(key,val)} style={{padding:"3px",border:"none",verticalAlign:"top",cursor:"pointer",position:"relative",background:"#1c3048",height:96,minWidth:110}}>
                {isEd?<input autoFocus value={editValue} onChange={e=>setEditValue(e.target.value)} onBlur={commitEdit} onKeyDown={e=>e.key==="Enter"&&commitEdit()} style={{width:"100%",height:"100%",fontSize:12,padding:"6px 8px",border:"none",borderRadius:4,fontFamily:"inherit",background:"#fff",color:"#1c3048",boxSizing:"border-box"}}/>:
                val?<div style={{background:cls.bg,borderRadius:4,height:"100%",overflow:"hidden",boxSizing:"border-box"}}><div style={{color:cls.color,fontSize:12,fontWeight:700,padding:"6px 8px",lineHeight:1.4,wordBreak:"break-word"}}>{val}</div></div>:
