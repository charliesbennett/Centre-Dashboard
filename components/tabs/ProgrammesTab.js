...
export default function ProgrammesTab({ groups, progStart, progEnd, centre, excDays, setExcDays }) {
  ...
  const renderCell = (key, val, onSite) => {
    const cls = classify(val);
    const isEd = editingCell === key;
    return (
      <td key={key} onDoubleClick={() => onSite && startEdit(key, val)} style={{
        padding: "1px 2px", borderLeft: key.endsWith("-AM") ? "2px solid " + B.border : "1px solid " + B.borderLight,
        verticalAlign: "middle", minWidth: 44, maxWidth: 80,
        background: !onSite ? "#f5f5f5" : cls.bg, cursor: onSite ? "pointer" : "default",
      }}>
        {isEd ? (
        ...
        ) : onSite ? <div style={{ height: 18 }} /> : <div style={{ height: 18 }} />}
      </td>
    );
  };
    ...
    <div>
      ...
      <div style={{ background: B.white, borderBottom: "1px solid " + B.border, padding: "10px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: B.navy }}>{dates.length} days &middot; {groups.length} groups</span>
        ...
        <div style={{ padding: "0 4px 16px", overflowX: "auto" }}>
          <TableWrap>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10 }}>
              ...
              <tbody>
                {groups.length === 0 ? (
                  <tr><td colSpan={100} style={{ textAlign: "center", padding: 36, color: B.textLight }}>Import groups in Students tab to see the overview</td></tr>
                ) : groups.map((g) => (
                  <tr key={g.id} style={{ borderBottom: "1px solid " + B.borderLight }}>
                    ...
                    {dates.map((d) => ["AM", "PM"].map((sl) => {
                      ...
                      return renderCell(key, grid[key], inRange(s, g.arr, g.dep));
                    }))}
                  </tr>
                ))}
              </tbody>
          ...
          <div style={{ padding: "6px 12px", fontSize: 9, color: B.textMuted }}>Double-click any cell to edit &middot; Click date headers to toggle excursion days</div>
        </div>
      )}
                  ...
                  <thead>
                    <tr>
                      ...
                      {dates.filter((d) => inRange(dayKey(d), selGroup.arr, selGroup.dep)).map((d) => {
                        ...
                        return (
                          <th key={s} style={{ ...thStyle, textAlign: "center", minWidth: 80, background: exc ? "#fff7ed" : we ? "#fef2f2" : "#f8fafc" }}>
                            <div style={{ fontSize: 7, color: B.textMuted }}>{fmtDate(d)}</div>
                            <div style={{ fontWeight: 800, fontSize: 9, color: we ? B.red : B.navy }}>{dayName(d)}</div>
                          </th>
                        );
        ...
        <div>
          ...
          {activeTemplate && (
            <div style={{ padding: "0 12px 16px" }}>
              <div style={{ padding: "8px 8px 4px", fontSize: 10, color: B.textMuted }}>{activeTemplate.centre} &middot; {activeTemplate.nights} &middot; {activeTemplate.period}</div>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10 }}>
                      <thead>
                        <tr>
                          <th style={{ ...thStyle, width: 30 }}></th>
                          {wk.days.map((d, i) => (
                            <th key={i} style={{ ...thStyle, textAlign: "center", minWidth: 100 }}>
