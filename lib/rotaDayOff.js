...
function preAssignFttFdeDayOffs(env) {
  // FTTs: day off on all weekends + any FDE weekday. 5FTTs: day off on all weekends.
  ...
  allStaff.forEach((s) => {
    ...
    onSiteDates(s, env.allDates).forEach((ds) => {
      ...
      if (isWeekend || (s.role === "FTT" && isFde)) setDayOff(env.grid, s.id, ds);
    });
  });
}
...
// FTT: handled entirely by preAssignFttFdeDayOffs (all weekends + FDE days).
const WEEKLY_DAYOFF_SKIP = new Set(["FTT"]);

function assignWeeklyDayOffs(env) {
  env.staff.filter((s) => !WEEKLY_DAYOFF_SKIP.has(s.role)).forEach((s) => {
    const weekendOnly = s.role === "TAL";
