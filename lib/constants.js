...
export const B = {
  ...
  cyan:    "#0891b2", cyanBg:   "#e0f2fe",
};

// Dark theme palette
export const DARK_B = {
  navy: "#1c3048",      // brand — unchanged
  red: "#ec273b",       // brand — unchanged
  white: "#ffffff",     // always-white (text on dark nav/headers)
  yellow: "#f0f279",    // brand — unchanged
  pink: "#3b1520",
  ice: "#1a2535",
...
export const SESSION_TYPES = {
  Lessons: "#3b82f6", "Multi-Activity": "#059669", "English+": "#0891b2",
  ...
  "Performing Arts": "#db2777", Football: "#15803d",
};
...
// Lesson slot for a group on a given date — flips each week for ZZ centres only
export const getGroupLessonSlot = (g, ds, isZZ = true) => {
  if (!g.arr || !g.lessonSlot) return g.lessonSlot || "AM";
  if (!isZZ) return g.lessonSlot;
  const daysSince = Math.floor((new Date(ds) - new Date(g.arr)) / 86400000);
  const weekNum = Math.floor(Math.max(0, daysSince - 1) / 7);
