...
const FILL_ELIGIBLE = new Set(["TAL", "SAI", "AL", "EAL", "SC", "AC", "EAC", "LAL", "LAC", "FOOTBALL", "PA", "HP"]);
...
function fillIdleSlots(ds, env) {
  // After demand allocation, fill remaining empty AM/PM slots with Activities so
  // no working day has blank cells — every staff member is visibly assigned.
  const { staff, grid, sessions } = env;
  staff.filter((s) => FILL_ELIGIBLE.has(s.role)).forEach((s) => {
    ...
    ["AM", "PM"].forEach((sl) => {
      ...
      place(grid, sessions, s.id, ds, sl, "Activities");
    });
  });
}
