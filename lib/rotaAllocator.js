...
function sortedSlotCells(dayDemand) {
  ...
  const isAMExcursion = (x) => x.slot === "AM" && x.cell.kind === "Excursion";
  const isActivity = (x) => x.cell.kind === "Activities";
  return combined.sort((a, b) => {
    if (isAMExcursion(a) !== isAMExcursion(b)) return isAMExcursion(a) ? -1 : 1;
    if (isActivity(a) !== isActivity(b)) return isActivity(a) ? 1 : -1;
    return (KIND_PRIORITY[a.cell.kind] || 9) - (KIND_PRIORITY[b.cell.kind] || 9);
  });
}
...
function fillIdleSlots(ds, env) {
  ...
  const { staff, grid, sessions, profiles, demand } = env;
  ...
  staff.filter((s) => s.role === "FTT").forEach((s) => {
    if (!isOnSite(s, ds) || isDayOff(grid, s.id, ds)) return;
    ["AM", "PM"].forEach((sl) => {
      if (!cellVal(grid, s.id, ds, sl)) grid[keyOf(s.id, ds, sl)] = "Office";
    });
  });
}
