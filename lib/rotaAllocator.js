...
function fillIdleSlots(ds, env) {
  ...
  const { staff, grid, sessions, profiles, demand, eveRoster, isINPS } = env;
  const profile = profiles?.[ds];
  const hasActiveStudents = profile?.activeStudents > 0 && !profile?.isFDE;
  if (hasActiveStudents && !isINPS) {
  ...
  staff.filter((s) => s.role === "FTT").forEach((s) => {
    ...
    ["AM", "PM"].forEach((sl) => {
      ...
      if (hasActiveStudents && daySessionCount(grid, s.id, ds) < 2) {
...
function allocateDay(ds, env) {
  ...
  assignEve(ds, env);
  if (!env.isINPS) fillSpecialistSlots(ds, env);
...
export function allocateRota({ staff, demand, bindings, fixedGrid = {}, dates, profiles = {}, isINPS = false }) {
  ...
  const shortfalls = [];
  const env = { staff, demand, bindings, grid, sessions, shortfalls, eveRoster: new Set(), profiles, isINPS };
