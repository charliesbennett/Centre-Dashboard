...
// Known aliases → expanded name for matching.
// Keys are the NORMALISED form (lowercased, apostrophes/punctuation removed).
// Values are space-separated significant words that match the DB centre name.
const CENTRE_ABBREV = {
  // Abbreviations used as DB centre names
  "qmu":                          "queen mary university london",
  "kcl":                          "kings college london",
  // Excel names that don't share words with short DB names
  "cheshire campus":              "crewe cheshire",
  "cheshire campus crewe":        "crewe cheshire",
  "cheshire":                     "crewe cheshire",
  "chethams school":              "manchester chetham",
  "chetham school":               "manchester chetham",
  "chethams school manchester":   "manchester chetham",
  // Other abbreviations / sheet codes
  "qw":   "queenswood",
  "uop":  "portsmouth",
  "rqa":  "queen annes",
  "bcc":  "bootham",
  ...
  "wasp": "wycombe abbey",
  "nrh":  "reaseheath",
  "chc":  "chetham manchester",
  "mcs":  "manchester chetham",
};

// Match a centre name string against the known centres list.
// Returns { centre, score } or null if no match found.
function matchCentreScored(raw, centres) {
  if (!raw) return null;
  const trimmed = raw.trim();
  // Normalise first so apostrophes/punctuation don't cause missed alias lookups
  const normRaw = normaliseCentreName(trimmed);
  const expanded = CENTRE_ABBREV[normRaw] || trimmed;
  ...
  for (const c of centres) {
    const dbExpanded = CENTRE_ABBREV[normaliseCentreName(c.name)] || c.name;
    const cn = normaliseCentreName(dbExpanded);
    const cnWordSet = new Set(cn.split(/\s+/).filter((w) => w.length > 3));
    const wordSet = new Set(words);
    // Use exact word matching (Set membership) — prevents "queen" matching "queenswood"
    const fwd = words.filter((w) => cnWordSet.has(w)).length;
    const rev = [...cnWordSet].filter((w) => wordSet.has(w)).length;
    const total = Math.max(wordSet.size + cnWordSet.size, 1);
    const score = (fwd + rev) / total;
    if (score > bestScore) { bestScore = score; best = c; }
  }
  return bestScore > 0 ? { centre: best, score: bestScore } : null;
}

function matchCentre(raw, centres) {
  return matchCentreScored(raw, centres)?.centre || null;
}
...
export async function parseTeamExcel(file, centres) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Centre: try col 12 first; if it doesn't produce a confident match (typos etc)
    // fall back to the section header match which is more reliable.
    const col12 = r[12]?.toString().trim();
    const centreRaw = col12 || currentCentreName;
    let centreMatch = currentCentreMatch;
    if (col12) {
      const scored = matchCentreScored(col12, centres);
      if (scored) centreMatch = scored.centre;
      // If col12 gave no match at all, keep currentCentreMatch as fallback
    }
