...
function matchCentre(raw, centres) {
  if (!raw) return null;
  const norm = normaliseCentreName(raw);
  ...
  for (const c of centres) {
    ...
    const matches = words.filter((w) => cn.includes(w)).length +
                    cnWords.filter((w) => norm.includes(w)).length;
    if (matches > bestScore) { bestScore = matches; best = c; }
  }
