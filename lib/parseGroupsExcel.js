...
// Check whether `abbr` (e.g. "qmu") is an acronym of the words in `fullWords`
// by matching each letter of the abbreviation to the first letter of successive words.
// Returns 0.85 if matched, 0 otherwise.
function acronymScore(abbr, fullWords) {
  if (abbr.length < 2 || abbr.length > 6 || abbr.includes(" ")) return 0;
  const letters = abbr.split("");
  let wi = 0;
  for (const letter of letters) {
    let found = false;
    while (wi < fullWords.length) {
      if (fullWords[wi][0] === letter) { wi++; found = true; break; }
      wi++;
    }
    if (!found) return 0;
  }
...
// Also handles: stemming ("Marys"→"Mary"), containment bonus, and acronym matching
// so "QMU" correctly matches "Queen Mary, University of London".
export function centreMatchScore(a, b) {
  ...
  // Acronym check: handles "QMU" ↔ "Queen Mary University of London"
  const acr = Math.max(acronymScore(wa.join(""), wb), acronymScore(wb.join(""), wa));

  return Math.max(contained ? Math.max(jaccard, 0.75) : jaccard, acr);
}
...
// Map raw Excel meal text to a dashboard MEALS value
function normaliseMeal(raw) {
  if (!raw) return null;
  const s = String(raw).toLowerCase().trim();
  if (s.includes("packed breakfast") || s.includes("packed bkfst")) return "Packed Bkfst";
  if (s.startsWith("breakfast")) return "Breakfast";
  if (s.startsWith("packed lunch")) return "Packed Lunch";
  if (s.startsWith("lunch")) return "Lunch";
  if (s.startsWith("packed dinner")) return "Packed Dinner";
...
export async function parseGroupsExcel(file, centres = []) {
  ...
  const arrMealCol    = col("Arr Meal");
  const depMealCol    = col("Dep Meal");
  ...
  for (let r = 2; r < rows.length; r++) {
    ...
    const firstMeal = normaliseMeal(row[arrMealCol]) || "Dinner";
    const lastMeal  = normaliseMeal(row[depMealCol]) || "Packed Lunch";
