...
export function useSupabase(centreId) {
  ...
  const saveRotaGrid = async (newGrid) => {
    ...
    Object.entries(newGrid).forEach(([key, value]) => {
      ...
      const cellDate = parts.slice(1, -1).join("-");
      rows.push({ centre_id: centreId, staff_id: staffId, cell_date: cellDate, slot, value });
    });
  ...
  const saveProgGrid = async (newGrid) => {
    ...
    Object.entries(newGrid).forEach(([key, value]) => {
      ...
      const cellDate = parts.slice(1, -1).join("-");
      rows.push({ centre_id: centreId, group_id: groupId, cell_date: cellDate, slot, value });
    });
