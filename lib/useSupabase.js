...
export function useSupabase(centreId) {
  ...
  const saveStaffMember = async (s) => {
    ...
    if (error) {
      ...
      if (typeof window !== "undefined") alert(`Staff save failed: ${error.message}\n\nYou may need to run staff_columns_migration.sql in Supabase.`);
    }
  };
  ...
  const saveTransfer = async (t) => {
    // Overflow fields (arr/dep split) stored as JSON in notes column
    const extras = {
      agent: t.agent || "",
      group: t.group || "",
      arrNotes: t.arrNotes || "",
      arrTerminal: t.arrTerminal || "",
      depAirport: t.depAirport || "Heathrow",
      depDate: t.depDate || "",
      depFlight: t.depFlight || "",
      depTime: t.depTime || "",
    ...
    const row = {
      ...
      type: "group",
      transfer_date: t.arrDate || null,
      transfer_time: t.arrTime || "",
      airport: t.arrAirport || "",
      flight_number: t.arrFlight || "",
      pax: t.pax || 0,
  ...
  const archiveProgramme = async (programmeName) => {
    ...
    const { error: archErr } = await supabase.from("programme_archives").insert({ centre_id: centreId, programme_name: programmeName, metadata: meta, data: snapshot });
    if (archErr) throw new Error("Archive save failed: " + archErr.message);
    // Wipe all programme data
    const groupIds = groups.map((g) => g.id);
    if (groupIds.length > 0) await supabase.from("students").delete().in("group_id", groupIds);
    await Promise.all([
      supabase.from("groups").delete().eq("centre_id", centreId),
      supabase.from("staff").delete().eq("centre_id", centreId),
      supabase.from("rota_cells").delete().eq("centre_id", centreId),
      supabase.from("programme_cells").delete().eq("centre_id", centreId),
      supabase.from("excursion_days").delete().eq("centre_id", centreId),
      supabase.from("excursions").delete().eq("centre_id", centreId),
      supabase.from("transfers").delete().eq("centre_id", centreId),
      supabase.from("programme_settings").delete().eq("centre_id", centreId),
      supabase.from("rooming_assignments").delete().eq("centre_id", centreId),
      supabase.from("rooming_rooms").delete().eq("centre_id", centreId),
      supabase.from("rooming_houses").delete().eq("centre_id", centreId),
    ]);
    // Reset local state
  ...
  const loadArchives = async () => {
    ...
    if (error) throw new Error("Load archives failed: " + error.message);
    return data || [];
  };

  const deleteArchive = async (archiveId) => {
    const { error } = await supabase.from("programme_archives").delete().eq("id", archiveId);
    if (error) throw new Error("Delete archive failed: " + error.message);
  };
