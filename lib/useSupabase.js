...
async function api(path, method = "GET", body = undefined) {
  ...
  if (res.status === 401) {
    try {
      localStorage.setItem("uklc_session_expired", "1");
      localStorage.removeItem("uklc_user");
    } catch {}
...
// Excursion bookings: a date can hold several bookings (different attractions,
// groups, or AM/PM splits), each optionally linked to Groups for auto headcounts.
function mapExcursionRow(r) {
  return {
    id: r.id, date: r.exc_date,
    groupIds: r.group_ids || [],
    attraction: r.attraction || "",
    dayPart: r.day_part || "Full",
    transportMethod: r.transport_method || "",
    manualStudentCount: r.manual_student_count || 0,
    manualLeaderCount: r.manual_leader_count || 0,
...
function toExcursionRow(e, centreId) {
  return {
    id: e.id, centre_id: centreId, exc_date: e.date,
    group_ids: e.groupIds || [],
    attraction: e.attraction || "",
    day_part: e.dayPart || "Full",
    transport_method: e.transportMethod || "",
    manual_student_count: e.manualStudentCount || 0,
    manual_leader_count: e.manualLeaderCount || 0,
    staff_count: e.staffCount || 0,
    booking_ref: e.bookingRef || "",
    email_contact: e.emailContact || "",
    booking_link: e.bookingLink || "",
    notes: e.notes || "",
    coaches: e.coaches || [],
  };
}
...
export function useSupabase(centreId) {
  ...
  const loadCentre = useCallback(async (cid) => {
    ...
    try {
      ...
      // Excursion bookings
      const excList = (d.excursions || []).map(mapExcursionRow);
    ...
    const ch = supabase
      ...
      .on("postgres_changes", {
        ...
        if (payload.eventType === "DELETE") {
          ...
          const mapped = mapExcursionRow(payload.new);
          setExcursions((prev) => {
            const exists = prev.find((e) => e.id === mapped.id);
            return exists ? prev.map((e) => e.id === mapped.id ? mapped : e) : [...prev, mapped];
          });
          const idx = lastSavedExcursionsRef.current.findIndex((e) => e.id === mapped.id);
  ...
  // ── SAVE: Excursion bookings (diff-based) ─────────────
  const saveExcursions = async (newExcursions) => {
    ...
    const toUpsert = newExcursions
      ...
      .map((e) => toExcursionRow(e, centreId));
