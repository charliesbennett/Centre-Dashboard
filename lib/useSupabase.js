...
export function useSupabase(centreId) {
  ...
  const loadCentre = useCallback(async (cid) => {
    ...
    try {
      ...
      setRoomingAssignments((assignData || []).map((a) => ({
  ...
  const saveProgGrid = async (newGrid) => {
    ...
    Object.entries(newGrid).forEach(([key, value]) => {
      ...
      const m = key.match(keyRegex);
      if (!m) return;
      const row = { centre_id: centreId, group_id: m[1], cell_date: m[2], slot: m[3], value };
      if (m[3] === "EVE") eveRows.push(row);
      else amPmRows.push(row);
    });

    // AM/PM rows saved in their own batches (always succeed)
    for (let i = 0; i < amPmRows.length; i += 500) {
      const { error } = await supabase.from("programme_cells").insert(amPmRows.slice(i, i + 500));
      if (error) console.error("saveProgGrid AM/PM:", error);
    }

    // EVE rows in separate batches — requires the eve_migration.sql to have been run
    for (let i = 0; i < eveRows.length; i += 500) {
      const { error } = await supabase.from("programme_cells").insert(eveRows.slice(i, i + 500));
      if (error) console.error("saveProgGrid EVE (run eve_migration.sql in Supabase):", error);
    }
  };
  ...
  // ── SAVE: Rooming Assignments (bulk per room) ─────────
  const saveRoomingAssignments = async (newAssignments) => {
    if (!centreId) return;
    await supabase.from("rooming_assignments").delete().eq("centre_id", centreId);
    const rows = newAssignments.filter((a) => a.occupantName || a.groupId).map((a) => ({
    ...
    if (rows.length > 0) {
      const { error } = await supabase.from("rooming_assignments").insert(rows);
      if (error) console.error("saveRoomingAssignments:", error);
    }
  };
