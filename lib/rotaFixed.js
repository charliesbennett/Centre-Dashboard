...
export function applyFixedForStaff(fixed, s, allDateStrs, groupArrivalDate, tos, isFullDayOff, inductionDates) {
  ...
  } else if ((inductionDates || []).length === 0 || isLateJoiner || attendable.length === 0) {
    // No configured dates, late joiner, or all induction dates are before this staff member's arrival:
    // place Induction on their first on-site day so it always precedes any Setup.
  ...
  if (!SPECIALIST_ROLES.has(s.role)) {
    ...
    // Only applies when staff arrive strictly before group arrival (they were on site for setup).
