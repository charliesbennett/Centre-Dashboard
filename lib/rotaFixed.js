...
export function applyFixedForStaff(fixed, s, allDateStrs, groupArrivalDate, tos, isFullDayOff, inductionDates) {
  ...
  // Induction dates this staff member can attend.
  // Staff whose contract starts exactly on group arrival day attend all configured induction
  // dates pre-contract (e.g. KCL: induction Jul 5, contract starts Jul 6 = group arrival).
  // All other staff only attend dates on or after their contracted arrival.
  const attendable = (inductionDates || []).filter((ds) => {
    if (!arrDs || ds >= arrDs) return true;
    return arrDs === groupArrivalDate;
  });
  ...
  if (inRange_.length > 0) {
    inductSet = new Set(inRange_);
             ...
             (attendable.length === 0 && arrDs && groupArrivalDate && arrDs < groupArrivalDate)) {
    // No configured dates, true late joiner, or all induction dates are before arrival AND staff
    // arrives before groups (they missed induction entirely — use first on-site day as fallback).
    inductSet = new Set(onSite[0] ? [onSite[0]] : []);
  } else {
    // Staff whose contracted start is on/after group arrival, with induction before that start.
    // They attend induction pre-contract — show configured dates in this fortnight.
    const preContract = (inductionDates || []).filter((ds) => allDateStrs.includes(ds));
    inductSet = new Set(preContract);
  }
  // If staff arrive before the first scheduled induction date, give them an informal induction
  // on their first on-site day so they aren't doing Setup before any induction at all.
  if (onSite[0] && inRange_.length > 0 && onSite[0] < inRange_[0]) inductSet.add(onSite[0]);
  ...
  if (!SPECIALIST_ROLES.has(s.role)) {
    ...
    // Applies when staff arrive on or before group arrival (they were on site during setup).
    if (arrDs && groupArrivalDate && arrDs <= groupArrivalDate) allDateStrs.forEach((ds) => {
      if (!lastInductDs || ds <= lastInductDs) return;
      if (ds >= arrDs) return;
      if (ds >= groupArrivalDate) return;
...
export function buildFixedGrid(staff, allDateStrs, groupArrivalDate, progYear, centreName) {
  ...
  // Normalise: RotaTab passes Date objects from genDates(); tests pass strings. Accept both.
  const dateStrs = allDateStrs.map((d) => typeof d === "string" ? d : d.toISOString().split("T")[0]);
  // RotaTab passes progStart ("2026-07-06") as progYear; parseTimeOff only needs the year.
  const year = String(progYear || "").slice(0, 4);
  staff.forEach((s) => {
    const tos = parseTimeOff(s.to, year);
    applyFixedForStaff(fixed, s, dateStrs, groupArrivalDate, tos, isFullDayOff, inductionDates);
  });
  return fixed;
}
