...
const CENTRES = [
  ...
  { aliases: ["kcl", "kings college", "kings"], dates: ["2026-07-05"] },
  { aliases: ["clifton"], dates: ["2026-07-05", "2026-07-06"] },
  { aliases: ["dean close"], dates: ["2026-07-05", "2026-07-06"] },
  { aliases: ["kingston"], dates: ["2026-07-06", "2026-07-07"] },
  { aliases: ["swithun"], dates: ["2026-07-10", "2026-07-11"] },
];

// Remove apostrophes/quotes entirely (so "King's" → "kings"), then replace
// remaining non-alphanumeric chars with spaces, so em-dashes etc. never block a match.
...
function matchCentre(centreName) {
  ...
  const n = normalise(centreName);
  return CENTRES.find((c) => c.aliases.some((a) => n.includes(normalise(a)))) || null;
}
