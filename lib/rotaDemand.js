...
function isExcursionVal(val) {
  return val && !/english\s*test|placement\s*test/i.test(val) && !/arriv|depart/i.test(val);
}
...
function slotProfile(env, ds, slot) {
  ...
  groups.forEach((g) => {
    ...
    if (!weekend && getGroupLessonSlot(g, ds) === slot) lessonStu += pax;
  });
...
function lessonsCells(env, ds, slot) {
  if (isWeekendDs(ds)) return [];
  const { groups, progGrid } = env;
  return groups
    .filter((g) => {
      if (!activeOnDate(g, ds) || ds === g.arr) return false;
      if (getGroupLessonSlot(g, ds) !== slot) return false;
      if (!(g.stu || g.gl)) return false;
      return !isExcursionVal(progVal(progGrid, g, ds, slot));
    })
    .map((g) => {
      const pax = (g.stu || 0) + (g.gl || 0);
      return {
...
function testingCells(profile, slot) {
  ...
  return [{
    kind: "Testing", label: "English Test",
    need: Math.ceil(slotP.testStu / STUDENTS_PER_TEACHER),
    prefer: PREFER_TEST, studentCount: slotP.testStu,
  }];
}
...
function slotCells(env, profile, slot) {
  ...
  if (profile.isTestingDay) return testingCells(profile, slot);
  return [...lessonsCells(env, profile.ds, slot), ...excursionCells(profile, slot)];
}
