...
export const PREFER_PICKUP = [...GENERAL_ACTIVITY_ROLES, "TAL"];
...
function slotProfile(env, ds, slot) {
  ...
  groups.forEach((g) => {
    ...
    if (isExplicitLesson || (!weekend && getGroupLessonSlot(g, ds, isZZ) === slot)) lessonStu += pax;
  });
...
function lessonsCells(env, ds, slot) {
  ...
  return groups
    .filter((g) => {
      ...
      if (!isExplicitLesson && getGroupLessonSlot(g, ds, isZZ) !== slot) return false;
...
export function buildDemand({ groups, progGrid = {}, progStart, progEnd, isZZ = true }) {
