...
export const MAX_EVE_STAFF = 4;

const ACTIVITY_ROLES = ["LAL", "LAC", "SAI", "EAL", "SC", "EAC", "FOOTBALL", "PA", "HP"];
const GENERAL_ACTIVITY_ROLES = ["LAL", "LAC", "SAI", "EAL", "SC", "EAC", "HP"];
export const PREFER_LESSONS = ["boundTAL", "FTT", "TAL", "5FTT"];
export const PREFER_EXCURSION = [...GENERAL_ACTIVITY_ROLES, "TAL"];
const PREFER_ACTIVITIES = ["FOOTBALL", "PA", ...GENERAL_ACTIVITY_ROLES, "TAL"];
export const PREFER_EVE = ["TAL", ...GENERAL_ACTIVITY_ROLES];
export const PREFER_PICKUP = [...GENERAL_ACTIVITY_ROLES, "TAL", "FTT", "5FTT"];
...
function isExcursionVal(val) {
  ...
  if (/multi.?activit|chosen\s*programme/i.test(val)) return false;
  return true;
}
...
function slotProfile(env, ds, slot) {
  const { groups, progGrid, isZZ } = env;
  ...
  groups.forEach((g) => {
    ...
    if (isExplicitLesson || (!weekend && (isZZ || getGroupLessonSlot(g, ds) === slot))) lessonStu += pax;
  });
...
function lessonsCells(env, ds, slot) {
  ...
  return groups
    .filter((g) => {
      ...
      // ZZ centres: lessons in both slots every day — skip the slot-flip filter
      if (!isZZ && !isExplicitLesson && getGroupLessonSlot(g, ds) !== slot) return false;
...
function activitiesCells(profile, slot) {
  ...
  if (actStu <= 0) return [];
  return [{
    kind: "Activities", label: "Multi-Activity",
    need: Math.ceil(actStu / STUDENTS_PER_STAFF),
    prefer: PREFER_ACTIVITIES, studentCount: actStu,
  }];
}
...
function eveCells(profile) {
  ...
  return [{
    ...
    need: Math.min(MAX_EVE_STAFF, Math.max(MIN_EVE_STAFF, Math.ceil(profile.students / STUDENTS_PER_STAFF))),
...
export function buildDemand({ groups, progGrid = {}, progStart, progEnd, isZZ = false, isINPS = false }) {
  ...
  const env = { groups: gs, progGrid, arrivalDates, groupArrivalDate, isZZ, isINPS };
