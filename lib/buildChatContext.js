...
export function buildChatContext(centreData, centreName) {
  ...
  if (excursions.length === 0) {
    ...
    excursions.forEach((exc) => {
      ...
      const attraction = exc.attraction || "—";
      const groupNames = (exc.groupIds || [])
        .map((id) => groups.find((g) => g.id === id)?.group)
        .filter(Boolean)
        .join(", ") || "—";
      const coaches = (exc.coaches || []).map((c) => c.company).filter(Boolean).join(", ") || "—";
      lines.push(`- ${dateStr} (${exc.dayPart || "Full"}): ${attraction} — groups: ${groupNames}, transport: ${exc.transportMethod || "—"}, coaches: ${coaches}`);
    });
  }
  lines.push("");
